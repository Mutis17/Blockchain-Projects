...
contract BBSEBank {
  ...
  */
  ...
  // Minimum duration threshold to avoid 0 interest for very short duration
  ...
  */
  constructor (address _bbseTokenContract, uint32 _yearlyReturnRate) {
  ...
  */
  function withdraw() public {
    ...
    // If duration is too short, force a minimum duration
    if (depositDuration < MIN_DURATION) {
      depositDuration = MIN_DURATION;
    }
    ...
    */
    ...
    emit DebugInfo(interestPerSecond, interest, depositedAmount, depositDuration);
  ...
  // Event to log intermediate values for debugging
  event DebugInfo(uint interestPerSecond, uint interest, uint depositedAmount, uint depositDuration);
